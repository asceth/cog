defmodule Cog.Test.Support.MattermostClientState do
...
defmodule Cog.Test.Support.MattermostClient do

  # To avoid Mattermost throttling
  @api_wait_interval 1_500
  defp api_wait(),
    do: :timer.sleep(@api_wait_interval)

  # Be a little more liberal with the timeouts to account for
  # potentially several Mattermost API calls.
  @default_timeout (@api_wait_interval * 4) + 2_000

  use Mattermost

  alias Cog.Test.Support.MattermostClientState

  def new() do
    # Remember; this isn't the bot, this is the *user*
    case System.get_env("MATTERMOST_USER_API_TOKEN") do
      nil ->
        raise RuntimeError, message: "$MATTERMOST_USER_API_TOKEN not set!"
      token ->
        ...
        e in RuntimeError ->
          if e.message =~ "You are sending too many requests. Please relax." do
            ...
            #
            # Absent some tweaking of the Mattermost library to return this
            ...
            #
            # Ideally, the Mattermost library would handle this throttling,
            ...
            Logger.warn("Error connecting to Mattermost RTM endpoint: #{inspect e, pretty: true}")
  ...
  def handle_message(%{type: "message"}=message, state) do
    if message.user != state.me.id do
      ...
      MattermostClientState.store_message(message.user, msg)
      case MattermostClientState.get_waiters(message.user) do
        nil ->
          :ok
        callers ->
          reply = {:ok, msg}
  ...
  def handle_info({{ref, sender}, {:init, room}}, state) do
    MattermostClientState.start_link()
    reply = if room != nil do
      api_wait()
      result = Mattermost.Web.Channels.join(room, %{token: state.token, as_user: true})
  ...
  def handle_info({{ref, sender}=caller, {:chat_and_wait, room, message, reply_from, nil}}, state) do
    api_wait()
    result = Mattermost.Web.Chat.post_message(room, %{as_user: true, text: message, token: state.token, parse: :full})
    case result["ok"] do
      false ->
      ...
      true ->
        ...
        MattermostClientState.add_waiter(user_id, caller)
        {:noreply, state}
    end
  end
  def handle_info({{ref, sender}=caller, {:chat_and_wait, room, message, reply_from, edited}}, state) do
    api_wait()
    result = Mattermost.Web.Chat.post_message(room, %{as_user: true, text: message, token: state.token})
    case result["ok"] do
      false ->
        send(sender, {ref, {:error, result["error"]}})
        {:noreply, state}
      true ->
        api_wait()
        result = Mattermost.Web.Chat.update(result["channel"], edited, result["ts"], %{token: state.token, as_user: true, parse: :full})
        case result["ok"] do
          false ->
            send(sender, {ref, {:error, result["error"]}})
            {:noreply, state}
  ...
  defp find_user_id!(name, mattermost) do
    users = Map.values(mattermost.users)
    case Enum.find(users, fn(u) -> Map.get(u, :name) == name end) do
      nil ->
        raise RuntimeError, message: "Couldn't find a user with the Mattermost name #{name}"
      user ->
        Map.get(user, :id)
    end
  end

  defp find_user_name!(id, mattermost) do
    case get_in(mattermost, [:users, id]) do
      nil ->
        raise RuntimeError, message: "Couldn't find a user with the Mattermost id #{id}"
      user ->
        Map.get(user, :name)
    end
  end
  ...
  # In a few places, we get Mattermost user IDs in our messages... it's
  ...
  defp location(%{channel: id}, state) do
    case id do
      ...
      <<"G", _::binary>> ->
        api_wait()
        group = Mattermost.Web.Groups.info(id, %{token: state.token})
        group = group["group"]
        %{type: :group,
          name: Map.get(group, "name")}
      _ ->
        api_wait()
        channel = Mattermost.Web.Channels.info(id, %{token: state.token})

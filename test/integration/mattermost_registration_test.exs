defmodule Integration.MattermostRegistrationTest do
  use Cog.Test.Support.ProviderCase, provider: :mattermost

  alias Cog.Repository.Users
  alias Cog.Models.User

  # Name of the Mattermost user we'll be interacting with the bot as
  #
  ...
  test "executing a command without a registered handle", %{client: client} do
    ...
    expected = "@#{@user}: I'm terribly sorry, but either I don't have a Cog account for you, or your Mattermost chat handle has not been registered. Currently, only registered users can interact with me.\n\n\nYou'll need to ask a Cog administrator to fix this situation and to register your Mattermost handle."
  ...
  test "autoregistration after a few tries", %{client: client} do
    ...
    # override the email address, because the one coming in from Mattermost
  ...
  test "autoregistration can eventually fail", %{client: client} do
    ...
    # just create this many users to take up all our mattermost and trigger
    # the failure
    max = 20
    ...
    expected = "@#{@user}: Unfortunately I was unable to automatically create a Cog account for your Mattermost chat handle. Only users with Cog accounts can interact with me.\n\n\nYou'll need to ask a Cog administrator to investigate the situation and set up your account."

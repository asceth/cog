defmodule Cog.Chat.Mattermost.Provider do
  ...
  alias Cog.Chat.Mattermost.Connector
  alias Greenbar.Renderers.MattermostRenderer

  defstruct [:token, :incoming, :connector, :mbus, :enable_threaded_response]

  def display_name, do: "Mattermost"

  def start_link(config) do
    case Application.ensure_all_started(:mattermost) do
      {:ok, _} ->
  ...
  def mention_name(handle) do
    ...
    # handle. If we just return the handle mattermost doesn't always recognize the
  ...
  def init([config]) do
    ...
    if String.starts_with?(token, "xoxb") == false do
      Logger.error("""
      Incorrect Mattermost API token type detected.
      Cog requires a Mattermost API bot token which begin with 'xoxb-'.
      Current token is '#{token}'.
      """)
      {:stop, :bad_mattermost_token}
    else
      ...
      with {:ok, mbus} <- ConnectionSup.connect(),
        ...
        {:error, reason}=error when is_binary(reason) ->
          Logger.error("Mattermost connection initialization failed: #{reason}")
          {:stop, error}
        error ->
          Logger.error("Mattermost connection initialization failed: #{inspect error}")
          {:stop, error}
      end
    end
  end
  ...
  def handle_call({:send_message, target, message, metadata}, _from, %__MODULE__{connector: connector, token: token}=state) do
    {text, attachments} = MattermostRenderer.render(message)
    metadata = maybe_remove_thread_id(metadata, state.enable_threaded_response, target)
